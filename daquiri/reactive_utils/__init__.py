...
class Transaction:
    ...
    def swap(cls, index: int, new_value: Any):
        return cls(
            kind=TransactionKind.Swap, message=IndexedTransaction(index, new_value)
        )

    @classmethod
    def add(cls, index: int = None, new_value: Any = None):
        return cls(
            kind=TransactionKind.Add, message=IndexedTransaction(index, new_value)
        )
...
class RxListPattern:
    ...
    def __init__(
        ...
        self.history = rx.merge(
            self.add, self.remove, self.reindex, self.clear, self.edit_at_index
        )
...
class RxTablePattern:
    ...
    @staticmethod
    def reduce_table_state(state: Tuple[pr.PVector, pr.PVector], tx: Transaction) -> Tuple[pr.PVector, pr.PVector]:
        """
    ...
    def __init__(self, columns, add_row=None, remove_row=None, edit_row_at_index=None, clear=None,
                 initial_rows=None):
        ...
        self._values_with_columns = self.history.pipe(
            ops.scan(self.reduce_table_state, (self.initial_rows, self.initial_columns)))
